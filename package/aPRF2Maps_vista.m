function [] = aPRF2Maps_vista(bidsfolder, subject, session, desc)
...
[~,id] = sort([d.datenum]);
d = d(id);
% pth = fullfile(bidsfolder, 'derivatives', 'prfanalyze-vista', desc, ['sub-' subject], ['ses-' session]);
...
load(fullfile(pth, sprintf('sub-%s_ses-%s_task-prf_acq-normal_run-99_results.mat', subject, session)), 'results');
...
mgz.vol = x0(leftidx);
MRIwrite(mgz, fullfile(pth, 'lh.x.mgz'));
mgz.vol = x0(rightidx);
MRIwrite(mgz, fullfile(pth, 'rh.x.mgz'));

mgz.vol = y0(leftidx);
MRIwrite(mgz, fullfile(pth, 'lh.y.mgz'));
mgz.vol = y0(rightidx);
MRIwrite(mgz, fullfile(pth, 'rh.y.mgz'));
